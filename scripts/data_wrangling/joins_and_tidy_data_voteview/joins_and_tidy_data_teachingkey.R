...
merged <- inner_join(members, immigration, by = join_by(state_abbrev, district_code == DISTRICT))
...
merged <- full_join(members, immigration, by = join_by(state_abbrev, district_code == DISTRICT))

filter(merged, is.na(Hispanic_pct))
filter(merged, is.na(district_code))
...
merged <- left_join(members, immigration, by = join_by(state_abbrev, district_code == DISTRICT))

####################
### right_join() ###
####################

merged <- right_join(members, immigration, by = join_by(state_abbrev, district_code == DISTRICT))
...
ggplot(merged, aes(x = nominate.dim2, y = Hispanic_pct)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

ggplot(merged, aes(x = nominate.dim2, y = FOREIGN_BORN_pct)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
...
merged |>
  pivot_longer(c(nominate.dim1, nominate.dim2),
             names_to = "dimension",
             values_to = "score") |>
  ggplot(aes(x = G20PREDBID_pct, y = score, color = dimension)) +
    geom_point() +
