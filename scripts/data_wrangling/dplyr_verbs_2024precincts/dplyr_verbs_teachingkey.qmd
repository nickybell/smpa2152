...
elec |>
  group_by(state) |>
  summarize(total_votes_dem = sum(votes_dem, na.rm = T),
...
elec |>
  ...
  ggplot() + 
  geom_col(aes(x = reorder(state, desc(total_dem_share)), y = total_dem_share)) + # You must arrange *within* a ggplot using reorder()!
...
elec |>
  filter(votes_total >= 25) |>
  group_by(state) |>
  summarize(avg_votes = mean(votes_total, na.rm = T),
            total_votes_dem = sum(votes_dem, na.rm = T),
            total_votes_rep = sum(votes_rep, na.rm = T),
            rep_two_party = total_votes_rep/(total_votes_rep+total_votes_dem)*100) |>
  ggplot(aes(x = avg_votes, y = rep_two_party)) +
  geom_point(color = "#DE0100") +
  geom_smooth(method = "lm", se = F, color = "#0A3161") +
  labs(x = "Average Number of Votes Per Precinct",
       y = "Republican Two-Party Vote Share (%)",
       title = "Average Precinct Size and the 2024 Presidential Election Results",
       caption = "Source: New York Times") +
  theme_classic() +
  theme(plot.title = element_text(hjust = .5))
```