...
5. Create a histogram showing the departure delays from JFK airport in hours (that is, use all flights from JFK, not just the ones delayed by 30 minutes or more). Each bar should represent 20 minutes (i.e., 1/3 of an hour).
...
6. Let's return to the original `flights` data frame now. Limit the data frame to only United Airlines (`UA`). Create a bar graph showing how many flights United Airlines took from each of the three NYC airports (JFK, LGA, and EWR).
